...
do
	...
	if [[ $ARCH == "linux-arm64"  ]]; then
		echo "linux-arm64"
		CC="zig cc -target aarch64-linux-gnu.${GLIBC_VERSION} -isystem /usr/include -L/usr/lib/aarch64-linux-gnu -O" CGO_LDFLAGS="-Wl,-Bstatic -lwebp -lwebpdemux -lwebpmux -Wl,-Bdynamic" CGO_ENABLED=1 GOOS=$RELEASE_PLATFORM GOARCH=$RELEASE_ARCH go build -ldflags="-s -w -X 'tidbyt.dev/pixlet/cmd.Version=${PIXLET_VERSION}'" -o build/${RELEASE_PLATFORM}_${RELEASE_ARCH}/pixlet tidbyt.dev/pixlet
	elif [[ $ARCH == "linux-amd64"  ]]; then
		echo "linux-amd64"
		CC="zig cc -target x86_64-linux-gnu.${GLIBC_VERSION} -isystem /usr/include -L/usr/lib/x86_64-linux-gnu -O" CGO_LDFLAGS="-Wl,-Bstatic -lwebp -lwebpdemux -lwebpmux -Wl,-Bdynamic" CGO_ENABLED=1 GOOS=$RELEASE_PLATFORM GOARCH=$RELEASE_ARCH go build -ldflags="-s -w -X 'tidbyt.dev/pixlet/cmd.Version=${PIXLET_VERSION}'" -o build/${RELEASE_PLATFORM}_${RELEASE_ARCH}/pixlet tidbyt.dev/pixlet
